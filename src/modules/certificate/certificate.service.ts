...
const generateVerificationHash = () => randomBytes(16).toString('hex');

const generateCertificatePdf = async (payload: {
  studentName: string;
  courseTitle: string;
...
const issueCertificate = async (enrollmentId: string) => {
  ...
  // Calculate grade from quiz attempts in this course
  ...
  // Ensure unique certificateNumber (small loop, low collision risk)
  ...
  // Make sure enrollment status is completed
...
const getCertificatePdf = async (certificateId: string) => {
