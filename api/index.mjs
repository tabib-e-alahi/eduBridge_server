import {
  ...
  prisma
} from "./chunk-R2GIAALA.mjs";
import {
  upload_service_default
} from "./chunk-DCRI3VTY.mjs";
import {
  config_default
} from "./chunk-TGI3OZ7A.mjs";
...
var globalErrorHandler = (err, req, res, next) => {
  ...
  return res.status(statusCode).json({
    ...
    message,
    errorSources,
    stack: config_default.env === "development" ? err?.stack : null
  });
};
...
var auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, token }) => {
      const EmailService = (await import("./email-CG75T6M3.mjs")).default;
...
var requirePermission = (...requiredPermissions) => {
  return catchAsync_default(async (req, res, next) => {
    ...
    if (!session) {
      return res.status(httpStatus2.UNAUTHORIZED).json({
        success: false,
    ...
    if (requiredPermissions.length && !hasAnyPermission(dbUser.role, requiredPermissions)) {
      return res.status(httpStatus2.FORBIDDEN).json({
        success: false,
...
var generateLearningPath = async (userId, payload) => {
  ...
  const NotificationService = (await import("./notification.service-ZUXC7A6K.mjs")).default;
...
var updateCourse = catchAsync_default(async (req, res) => {
  ...
  if (!course) {
    return sendResponse_default(res, {
      statusCode: httpStatus5.NOT_FOUND,
      success: false,
  ...
  if (course.instructorId !== userId && role !== "ADMIN") {
    return sendResponse_default(res, {
      statusCode: httpStatus5.FORBIDDEN,
      success: false,
...
var getUserDashboardData = async (userId) => {
  ...
  const weeklyActivity = last7Days.map((date) => {
    ...
    return { date, count: count || Math.floor(Math.random() * 5) };
  });
  return {
    enrolledCourses,
    savedCourses,
...
var createLesson = catchAsync_default(async (req, res) => {
  ...
  if (!course) {
    return sendResponse_default(res, {
      statusCode: httpStatus8.NOT_FOUND,
      success: false,
      message: "Course not found",
      data: null
    });
  ...
  if (course.instructorId !== userId && role !== "ADMIN") {
    return sendResponse_default(res, {
      statusCode: httpStatus8.FORBIDDEN,
      success: false,
...
var authMiddleware = (...requiredRoles) => {
  return catchAsync_default(async (req, res, next) => {
    ...
    if (!session) {
      return res.status(httpStatus9.UNAUTHORIZED).json({
        success: false,
    ...
    if (requiredRoles.length) {
      ...
      if (!dbUser || !requiredRoles.includes(dbUser.role)) {
        return res.status(httpStatus9.FORBIDDEN).json({
          success: false,
...
var deleteImage = catchAsync_default(async (req, res) => {
  ...
  const uploadService = (await import("./upload.service-HFDGVB5A.mjs")).default;
  const result = await uploadService.deleteImage(publicId);
  if (!result) {
    return sendResponse_default(res, {
      statusCode: httpStatus12.INTERNAL_SERVER_ERROR,
      success: false,
...
var createSupportTicketInDB = async (payload) => {
  ...
  const result = await prisma.report.create({
    data: {
      reporterId: payload.reporterId || "00000000-0000-0000-0000-000000000000",
      // System user or anonymous
...
var app = express18();
app.use(helmet());
var allowedOrigins = process.env.ALLOWED_ORIGINS?.split(",") || ["http://localhost:3000"];
...
app.get("/", (req, res) => {
  res.send("EduBridge AI Server is running! \u{1F680}");
});
